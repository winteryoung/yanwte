...
object YanwteContainer {
    ...
    private fun registerExtensionSpace(name: String, space: YanwteExtensionSpace) {
        ...
        extSpaceNameToDataExtInitializer[name] ?: run {
            createDataExtInitializer(name) { extSpaceName ->
                val classLoader = space.javaClass.classLoader
                try {
                    @Suppress("UNCHECKED_CAST")
                    (classLoader.loadClass("$extSpaceName.DataExtensionInitializer")
    ...
    internal fun createDataExtInitializer(
            extSpaceName: String,
            initializerClassLoader: (String) -> Class<YanwteDataExtensionInitializer>?
    ): YanwteDataExtensionInitializer {
        extSpaceNameToDataExtInitializer[extSpaceName]?.let {
            return it
        }

        return initializerClassLoader(extSpaceName)?.let {
            it.newInstance()
        }.let {
    ...
    /**
     * Get data extension initializer of the given extension space, if any.
     */
